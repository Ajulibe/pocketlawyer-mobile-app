...
const allFomKeys = [...Array(10).keys()].map((item, index) => ({
  ...{
    [`firstName${index + 1}`]: `FirstName${index + 1}`,
    [`lastName${index + 1}`]: `LastName${index + 1}`,
    [`meansOfId${index + 1}`]: `MeansOfIdentification${index + 1}`,
    [`idNo${index + 1}`]: `IdentityNumber${index + 1}`,
    [`signature${index + 1}`]: `Signature${index + 1}`,
  },
}));
